...
class DropDownMenu2 extends StatelessWidget {
  const DropDownMenu2({super.key, required this.index});
  final int index;
  @override
  ...
  builder: (context, state) {
    return DropdownButton2(
        customButton:  Icon(
          Icons.more_vert,
          size: 30,
          color: BlocProvider.of<TaskBloc>(context).isIndexChanged ? HexColor("FFFFFF") : HexColor("0000FF"),
        ),
        items: [
...
abstract class MenuItems {
  ...
  static const List<MenuItem> secondItems = [mark];

  static const important = MenuItem(text: 'Important', icon: Icons.label_important_outline,);
  static const  mark = MenuItem(text: 'Mark', icon: Icons.invert_colors_on);
  ...
  static void onChanged(BuildContext context, MenuItem item,state,int index) {
    ...
    switch (item) {
      ...
      case MenuItems.mark:
        {
          BlocProvider.of<TaskBloc>(context).add(ChangeIndexColor(
          ));
          break;
        }
    }
